...
#### Tasks completed: 
  ...
  * Took 1L and bottle samples from each tank. Since we are taking duplicate bottle samples per tank, we've developed this standard for location where we filled the bottle in the culture tank: A = front bottom, B = back top. The 1L samples were all taken in the front. 
  * Measured T, S & pH of 1L samples. 
  * Started injecting HgCl to all bottle samples, but Dispensette was not functioning properly- volume of HgCl dispensed was inadequately low, and not consistent. Spent the next ~1hr getting set up with a standard digital pipette.  Added HgCl to all bottles. The bottles that received a small volume during first injection attempt are:   T2-B, T2-A, T1-B, H1-B, H2-A
  ...
  * Feed each tank 500mL algae (from header tank) after cleaning 
